...
class SuperclusterImmutable<T> extends Supercluster<T> {
  ...
  Supercluster({
    ...
    this.extractClusterData,
      ...
      : minZoom = minZoom ?? 0,
        ...
        trees = List.generate(
          (maxZoom ?? 16) + 2,
              (_) => RBushPointTree<T>(maxEntries: 999999999999),
        ) {
    _load(points);
  }
  ...
  List<ClusterOrMapPoint<T>> getClustersAndPoints(double westLng,
    ...
    return trees[_limitZoom(zoom)].withinBounds(
      util.lngX(minLng),
      util.latY(maxLat),
      util.lngX(maxLng),
      util.latY(minLat),
  ...
  List<ClusterOrMapPoint<T>> getChildren(int clusterId) {
    ...
    for (final clusterOrPoint in index.withinRadius(origin.x, origin.y, r)) {
      if (clusterOrPoint.parentId == clusterId) {
        children.add(clusterOrPoint);
      }
    }
