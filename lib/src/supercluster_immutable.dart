...
import 'package:supercluster/src/kdbush_point_index.dart';

import '../supercluster.dart';

class SuperclusterImmutable<T> extends Supercluster<ClusterOrMapPoint<T>> {
  ...
  @override
  final List<KDBushPointIndex<T>> trees;
  ...
  @override
  void setClusters(int zoom, List<ClusterOrMapPoint<T>> clusters) {
    trees[zoom].load(clusters);
  }
  ...
  List<ClusterOrMapPoint<T>> withinBounds(
    ...
    return trees[treeIndex].withinBounds(minX, minY, maxX, maxY);
  }
